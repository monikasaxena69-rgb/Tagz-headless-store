...
export async function getProducts(): Promise<Product[]> {
  // Temporary bypass: return mock data if environment variables aren't set
  const domain = process.env.SHOPIFY_STORE_DOMAIN;
  const token = process.env.SHOPIFY_STOREFRONT_ACCESS_TOKEN;
    ...
    return [
      {
        ...
        description: 'This is a sample product. Add your Shopify credentials to see real products.'
      }
    ];
  }
